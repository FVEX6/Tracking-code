...
%得到目标模板的初始数据
I=imread("F:\桌面\转换文件\转换文件\ARITICLE DATA\TURTLE\20240412\1.png");
imshow(I);
rect = getrect();
x1 = rect(1); 
x2 = rect(1) + rect(3);
...
F = getframe;
mkdir("F:\桌面\转换文件\转换文件\ARITICLE DATA\TURTLE\result");
image_source=strcat('F:\桌面\转换文件\转换文件\ARITICLE DATA\TURTLE\20240412\','1.jpg');
...
for loop=2:n
    ...
    b=[a,'.png'];
    b=['F:\桌面\转换文件\转换文件\ARITICLE DATA\TURTLE\20240412\',b];
    I=imread(b);
    [H,S,V]=rgb_to_rank(I);
    %产生随机粒子
...
for new_x=round(Estimate(loop).x)-i:round(Estimate(loop).x+i)
       for new_y=round(Estimate(loop).y)-i:round(Estimate(loop).y+i)
          ...
          TargetPic(new_y,new_x,3)=0;
       end
end

     imshow(TargetPic);
     F = getframe;
     image_source=strcat("F:\桌面\转换文件\转换文件\ARITICLE DATA\TURTLE\result\",num2str(loop),'.png');
     imwrite(F.cdata,image_source);  
end
